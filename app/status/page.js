...
const Status = () => {
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    const totalShards = data.clusters.length * 16;
    const serverId = parseInt(searchTerm, 10);
    const shardId = Math.floor((serverId / 4194304) % totalShards);
    const cluster = data.clusters.find(cluster => cluster.shardsStats.some(shard => shard.id === shardId));

    if (cluster) {
      setSearchResult({ cluster: cluster.cluster, shardID: shardId });
    } else {
